  ...
  <div>
    <svg
      ...
      :style="{
        transform: `
        translate(${x}px, ${y}px)
        scale(${(offsetX + boxOffset.x) / Math.abs(offsetX + boxOffset.x)},${
          (offsetY + boxOffset.y) / Math.abs(offsetY + boxOffset.y)
        }) 
        translate(${
          offsetX + boxOffset.x > 0 ? 0 : -(offsetX + boxOffset.x)
        }px,${offsetY + boxOffset.y > 0 ? 0 : -(offsetY + boxOffset.y)}px)
        
        `,
        'z-index': `${z_index}`,
      }"
      :viewBox="`0 0 ${Math.abs(offsetX + boxOffset.x)} ${Math.abs(
        offsetY + boxOffset.y
      )}`"
    ...
    <div
      ...
      :style="{
        ...
        'z-index': `${z_index}`,
        transform: `translate(${x + offsetX + boxOffset.x}px, ${
          y + offsetY + boxOffset.y
        }px)`,
        ...style,
      }"
      ref="el"
...
async function onDragStart(e) {
  ...
  e.dataTransfer.setDragImage(el.value, e.offsetX, e.offsetY);
}
