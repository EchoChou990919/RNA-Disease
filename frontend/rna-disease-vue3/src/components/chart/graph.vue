<template>
        ...
        <n-space justify="space-between">
          <n-space>
            <perdict-glyph-vue
              ...
              :size="300"
              :perdict_value="
                connMtx[selectionStore.locked[0].id][node.id].value || 0
              "
              :perdict_label="
                connMtx[selectionStore.locked[0].id][node.id].value >= 0.5
                  ? 1
                  : 0
              "
              :truth_label="
                connMtx[selectionStore.locked[0].id][node.id].type == 2 ? 0 : 1
              "
              :palette="{
                0: 'blue',
                1: 'red',
            ...
            <pie-vue
              ...
              :data="[
                connTable[node.id].filter((e) => e.type == 1).length,
                connTable[node.id].filter((e) => e.type == 2).length,
                connTable[node.id].filter((e) => e.type == 3).length,
              ]"
              :colors="
                (a, b, index) => {
                  return {
          ...
          <div v-if="node.locked || node.showEvidence">
            <div v-if="node.showDAG" class="tooltip-dag">
              <disease-similarity-vue
                v-if="
                  selectionStore.locked[0].category == 0 && node.category == 0
                "
  ...
  >
    <g>
      <g>
        <!-- 节点链接图 -->
        <TransitionGroup name="edge" tag="g" v-if="renderDone">
          <g v-for="edge in batchedEdges" :key="edge.id + edge.color">
            <TransitionGroup name="edge" tag="g">
              <path
                v-for="path in edge.paths"
                :stroke-width="2 / transform.k"
                :key="edge.color + path.opacity"
                :opacity="path.opacity"
                :d="path.d"
                :stroke="edge.color"
                :stroke-dasharray="edge.type ? '1' : 'none'"
              />
            </TransitionGroup>
          </g>
        </TransitionGroup>
        ...
        <g v-if="renderDone" v-for="(node, index) in nodes" :key="node.id">
          <perdict-glyph-vue
            ...
            "
            :perdict_label="
              connMtx[selectionStore.locked[0].id][node.id].value >= 0.5 ? 1 : 0
            "
            :truth_label="
              connMtx[selectionStore.locked[0].id][node.id].type == 2 ? 0 : 1
            "
...
import {
  ...
  net2connTable,
  connMtx,
} from "@/service/dataloader/nodelinks";
...
import {
  loadDiseaseAttrs,
  doid2name,
} from "@/service/dataloader/diseaseDetail";
...
const x = d3
  .scaleLinear()
  .domain(RANGE_X)
  .range([VIEW_RANGE[0], VIEW_RANGE[2]]);
const y = d3
  .scaleLinear()
  .domain(RANGE_Y)
  .range([VIEW_RANGE[1], VIEW_RANGE[3]]);
const rx = d3
  .scaleLinear()
  .domain([VIEW_RANGE[0], VIEW_RANGE[2]])
  .range(RANGE_X);
const ry = d3
  .scaleLinear()
  .domain([VIEW_RANGE[1], VIEW_RANGE[3]])
  .range(RANGE_Y);
...
const showEdgesNew = computed(() => {
  const showNodes = _([...selectionStore.locked, selectionStore.hovered])
    .filter((n) => n != null)
    .uniqBy("id")
    .value();
  ...
  const firstMidNodes = new Set(
    firstEdges.filter((e) => e.type == 0).map((e) => e.target.id)
  );
  ...
  const midNodes = new Set(
    _.intersection([...firstMidNodes], [...otherMidNodes])
  );
  const firstEdgesMid =
    otherNodes.length > 0
      ? firstEdges.filter((e) => midNodes.has(e.target.id))
      : firstEdges;
  const directEdges = firstEdges.filter((e) => otherNode_set.has(e.target.id));

  const res = _.union(
    [...directEdges, ...firstEdgesMid, ...otherEdges].filter(
      (e) => e.source != e.target
    )
  );
...
const labels = computed(() => {
  ...
  const target = [
    ...selectionStore.locked.map((node) => ({
      ...node,
      locked: true,
    })),
    ...
    target
      ...
      .map((node) => {
        return {
          ...
          showEvidence:
            node.category == 1 && DAGSource && DAGSource.category == 0,
          ...node,
        };
      }),
    (node) => node.name
...
function removeLock(node) {
  ...
  highlight(
    [
      ...selectionStore.locked,
      selectionStore.hovered,
      ...showEdgesNew.value.map((e) => e.target),
    ].filter((e) => e != null)
  );
}
...
function showGlyph(node) {
  return (
    ...
    showGlyph &&
    isHighlight(node) &&
    selectionStore.locked.length >= 1 &&
    selectionStore.locked[0].category == 0 &&
    connMtx[selectionStore.locked[0].id][node.id] &&
    connMtx[selectionStore.locked[0].id][node.id].type != 0
  );
}

function hasEvidence(node) {
  return (
    (lncDisConnMtx.lncRNADisease[selectionStore.locked[0].name] &&
      lncDisConnMtx.lncRNADisease[selectionStore.locked[0].name][
        names[node.name]
      ] != null) ||
    (lncDisConnMtx.lncRNA2Cancer[selectionStore.locked[0].name] &&
      lncDisConnMtx.lncRNA2Cancer[selectionStore.locked[0].name][
        names[node.name]
      ] != null)
  );
...
const batchedEdges = computed(() => {
  // 一共是四种颜色的边，同类型的边会有不同的透明度，如果按照5个level离散化，
  // 那就是3+5一共8种线
  let batch = _(showEdgesNew.value)
    .groupBy(
      (edge) =>
        edge.source.id + (edgeColor(edge) || colors[edge.target.category])
    )
    .values()
    .map((edges) => {
      const isSameCategory =
        edges[0].source.category === edges[0].target.category;
      let target_edges = [edges];
      if (isSameCategory) {
        target_edges = _(edges)
          .groupBy((edge) => Math.floor(edge.value / 0.2))
          .values()
          .value();
      }
      const paths = target_edges.map((edges) => {
        return {
          opacity: Math.floor(edges[0].value / 0.2) * 0.1,
          d: _.reduce(
            edges,
            (path, edge) =>
              path +
              `M ${_.floor(x(edge.source.x), 3)} ${_.floor(
                y(edge.source.y),
                3
              )} L ${_.floor(x(edge.target.x), 3)} ${_.floor(
                y(edge.target.y),
                3
              )} `,
            ""
      ...
      return {
        paths,
        color: edgeColor(edges[0]) || colors[edges[0].target.category],
        type: edges[0].source.category === edges[0].target.category,
        id: edges[0].source.id,
      };
    })
    .value();
  console.log(batch);
